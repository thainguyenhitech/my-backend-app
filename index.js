...
const DB_CONFIG = {
  ...
  max: 10, // Giảm từ 20 để tối ưu kết nối
      ...
      query = `
        SELECT 
          ...
          (SELECT ARRAY_AGG(i.name) 
           FROM post_product_items i 
           WHERE i.post_id = p.post_id 
           ${searchTerm ? `AND i.name ILIKE $${paramIndex++}` : ''}) AS product_name
        FROM posts p
        WHERE EXISTS (
          SELECT 1 
          FROM post_product_items i 
          WHERE i.post_id = p.post_id
          ${searchTerm ? `AND i.name ILIKE $${paramIndex++}` : ''}
        )
      `;
  ...
  try {
    ...
    if (subcategoryId) {
      ...
      if (subcategoryResult.rows.length > 0) {
        subcategoryName = subdirectoryResult.rows[0].subcategory_name;
